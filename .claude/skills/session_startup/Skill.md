...
Read({
  file_path: "jarvis-memory/L0_Working/scratchpad.md",
  limit: 20
})
```
...
2. **habit_data.json**（完整加载，约 2.2k tokens）
   ```javascript
   Read({
     file_path: "jarvis-memory/L0_Working/habit_data.json"
   })
   ```
...
3. **.current_task.json**（完整加载，约 400 tokens）
   ```javascript
   Read({
     file_path: "jarvis-memory/L0_Working/.current_task.json"
   })
   ```
...
4. **next_actions_kanban.md**（如需要）
   ```javascript
   Read({
     file_path: "jarvis-memory/L0_Working/next_actions_kanban.md"
   })
   ```
...
// 尝试读取昨日日报 JSON
Read({
  file_path: "jarvis-memory/L1_Episodic/01-Daily/.data/{{yesterday}}.json"
})
