...
// MiniMax API 配置（从 jarvis_daemon.py 迁移）
const AI_API_BASE = process.env.CTI_AI_API_BASE || 'https://api.minimaxi.com/anthropic';
const AI_API_KEY = process.env.CTI_AI_API_KEY || '';
      ...
      start(controller) {
        ...
        (async () => {
          ...
          try {
            ...
            // Ensure CLAUDECODE is not in the env passed to subprocess
            delete cleanEnv.CLAUDECODE;
            // 设置 CLAUDE_CONFIG_HOME 使用临时配置
            cleanEnv.CLAUDE_CONFIG_HOME = configDir;
            ...
            // Debug logging
            console.error('[llm-provider] Debug - Using MiniMax API:', AI_API_BASE);
            console.error('[llm-provider] Debug - configDir:', configDir);
            console.error('[llm-provider] Debug - workingDirectory:', params.workingDirectory);
            ...
            try {
              if (tmpDir && fs.existsSync(tmpDir)) {
                ...
                console.error('[llm-provider] Debug - cleaned up temp dir:', tmpDir);
              }
            } catch (cleanupErr) {
              console.error('[llm-provider] Warning - failed to cleanup temp dir:', cleanupErr);
            }
          }
        })();
      },
