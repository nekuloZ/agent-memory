...
export function loadConfig(): Config {
  ...
  // Sync config file env vars to process.env (if not already set)
  // This ensures CTI_CLAUDE_CODE_EXECUTABLE and other vars are available
  for (const [key, value] of env.entries()) {
    if (key.startsWith('CTI_') && process.env[key] === undefined) {
