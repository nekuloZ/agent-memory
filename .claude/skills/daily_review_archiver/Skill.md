...
**⚠️ 跨平台用户必读：** 如果你在多个设备使用 git 同步记忆系统，归档前请确保已完成同步。

执行以下检查：

```bash
git status --porcelain jarvis-memory/L0_Working/
```
...
| 情况 | 输出 | 操作 |
|------|------|------|
| 有未同步修改 | ⚠️ 检测到未同步内容 | 提示用户先执行 `git pull` 或 `git push` |
| 无未同步修改 | ✅ 已是最新 | 继续 Step 1 |

**提示用户：**
```
⚠️ 检测到 L0_Working 有未同步的修改

【建议操作】
- 如果在其他设备工作过：先执行 git pull 拉取最新内容
- 如果本地有未提交的修改：先执行 git add + git commit
...
用 Read 工具读取：
- `jarvis-memory/L0_Working/scratchpad.md`
- `jarvis-memory/L0_Working/habit_data.json`
- `jarvis-memory/L0_Working/next_actions_kanban.md`
- `jarvis-memory/L0_Working/.current_task.json`
...
1. **生成 Daily Log**（Markdown）
   - 路径：`jarvis-memory/L1_Episodic/01-Daily/YYYY-MM-DD.md`
   - 包含：发现、决策、项目进展、数据
   ...
   **创建方式：**
   ...
   - **方式二（Obsidian 运行时）**：使用 CLI
     ```bash
     obsidian create path="jarvis-memory/L1_Episodic/01-Daily/YYYY-MM-DD.md" content="..."
     ```

2. **生成 JSON**（数据分析用）
   - 路径：`jarvis-memory/L1_Episodic/01-Daily/.data/YYYY-MM-DD.json`
